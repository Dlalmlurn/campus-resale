...
| 分组 | 对象 | 契约含义 |
| --- | --- | --- |
| 身份权限 | `User`, `Role`, `UserRole`, `CampusAuth`, `CampusAuthFactor` | 用户主体、角色绑定、校园认证记录、认证因子、可信度和审核结论。 |
...
| 支付资金 | `PaymentOrder`, `PaymentTransaction`, `PaymentCallbackLog`, `PayoutAccount`, `SettlementRecord`, `RefundOrder` | 支付单、支付流水、回调日志、收款方式、托管结算、全额退款和部分退款。 |
| 评价信用 | `Review`, `CreditRecord`, `CreditSummary` | 交易评价、信用变化事实和信用摘要。 |
...
| 接口族 | 稳定职责 |
| --- | --- |
| Identity API | 注册、登录、退出、会话续期、权限查询。 |
...
| Payment API | 创建支付单、查询支付状态、接收支付回调、关闭支付单、创建退款单、查询退款状态、创建结算记录。 |
...
| 事件族 | 覆盖事实 |
| --- | --- |
| Identity Events | 用户注册、认证提交、因子验证、可信度变化、认证通过、认证驳回、认证失效。 |
...
| Order Events | 订单创建、金额冻结、商品占用、卖家确认、卖家拒绝、订单取消、完成确认请求、买家确认、买家异议、超时确认、订单完成。 |
| Payment Events | 支付单创建、支付成功、支付失败、资金托管、支付回调处理、结算请求、结算完成、结算失败。 |
...
- 可信度达到 50 分进入管理员审核队列；达到 60 分且管理员审核通过后开放完整交易权限。
...
- 结算记录必须保存订单、支付单、结算金额、收款方式快照、结算流水号、结算时间、状态和失败原因。
...
| 退款申请入口 | 订单完成后 7 天 | 关闭新退款入口，已发起退款继续处理。 |
